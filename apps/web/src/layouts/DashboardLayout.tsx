...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'RAG Overview', path: '/', icon: LayoutDashboard },
    { name: 'Document Ingestion', path: '/ingestion', icon: FileText },
    { name: 'Vector Store', path: '/vectorstore', icon: Database },
    { name: 'Retrieval Engine', path: '/retrieval', icon: Network },
    { name: 'Generation (LLM)', path: '/generation', icon: Cpu },
    { name: 'Evaluation Suite', path: '/evaluation', icon: BarChart3 },
    { name: 'Prompt Patterns', path: '/prompts', icon: Layers },
    { name: 'RAG Observability', path: '/observability', icon: Zap },
    { name: 'Pipeline Graph', path: '/pipeline', icon: GitBranch },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-emerald-400 bg-clip-text text-transparent">RAG-ARCH</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">RAG Architect</p>
                <p className="text-xs text-slate-500 truncate">AI Systems Team</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Query RAG, search documents, or view logs..." 
